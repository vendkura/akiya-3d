...
def load_model():
    model = smp.FPN(encoder_name="resnet34", encoder_weights=None, in_channels=3, classes=13)
...
def plot_iou_per_class(class_ious, class_counts, output_path):
    ...
    ax.set_title('Per-Class IoU Performance - FPN Model', fontsize=14, fontweight='bold')
...
def plot_confusion_matrix(cm, output_path):
    ...
    ax.set_title('Confusion Matrix - FPN Model (Normalized by True Class)', fontsize=14, fontweight='bold')
...
def colorize_mask(mask, num_classes=13):
...
def generate_report(class_ious, class_counts, cm, output_path):
    ...
    with open(output_path, 'w') as f:
        f.write("="*70 + "\n")
        f.write("PER-CLASS IoU ANALYSIS REPORT - FPN MODEL\n")
        f.write("="*70 + "\n\n")
...
for i, img_path in enumerate(test_images, 1):
    # Find mask (with _mask suffix)
    mask_path = Path(TEST_MASKS_DIR) / (img_path.stem + "_mask.png")
