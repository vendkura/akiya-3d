...
for img_info in coco_data.get('images', []):
    ...
    # Extract actual filename from format: images\\{hash}__{hash}-{actual_name}.png
    from urllib.parse import unquote
    
    raw_filename = Path(img_filename).name  # Get just the filename
    
    # Remove hash prefixes: {hash}__{hash}-{actual_name}.png -> {actual_name}.png
    if '__' in raw_filename:
        # Split by '__' and take the second part
        after_double_underscore = raw_filename.split('__', 1)[1]
        # Now format is: {hash}-{actual_name}.png
        if '-' in after_double_underscore:
            # Remove the first hash and hyphen
            clean_filename = '-'.join(after_double_underscore.split('-', 1)[1:])
        else:
            clean_filename = after_double_underscore
    else:
        clean_filename = raw_filename
    
    # URL decode (e.g., %C3%A9 -> é)
    clean_filename = unquote(clean_filename)
    
    # Replace underscores with spaces to match actual image filenames
    clean_filename = clean_filename.replace('_', ' ')
    
    # Fix the specific French screenshot naming pattern
    # "Capture décran" should be "Capture d'écran" with Unicode right single quotation mark (U+2019)
    clean_filename = clean_filename.replace('Capture décran', "Capture d\u2019écran")
